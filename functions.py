...
class Transaction():
    ...
    def __init__(self, DataFrame, col_transact_name, col_time_name, col_identifier_name):
        ...
        self.id_col_name = col_identifier_name
    ...
    def group_case_by_time(self):
        self.df =  self.df.groupby(self.id_col_name).apply(lambda x: x.sort_values(self.time_col_name))
    
    def shift_process_col(self):
        self.df['concept:name:2'] = pd.DataFrame(self.df.groupby(self.id_col_name).apply(lambda x: \
                         x[self.tr_col_name].shift(periods=-1).fillna('Конец лога')))[self.tr_col_name].values
    
    def shift_time_col(self):
        self.df['time:timestamp:2'] = pd.DataFrame(self.df.groupby(self.id_col_name).apply(lambda x: \
                         x[self.time_col_name].shift(periods=-1).fillna(self.t_date)))[self.time_col_name].values
    
    def time_diff(cls):
        if 'time:timestamp:2' not in cls.df.columns:
            cls.shift_time_col()
        cls.df['time_diff'] = cls.df['time:timestamp:2'] - cls.df[cls.time_col_name]
        cls.df['time_diff'] = cls.df['time_diff'].apply(lambda x: 0 if x.days > 1000 else x.total_seconds())
    
    def transact(cls):
        if 'concept:name:2' not in cls.df.columns:
            cls.shift_process_col()
        cls.df['transact'] = cls.df[cls.tr_col_name]+'-->'+cls.df['concept:name:2']
...
class Prepare(Transaction):
    ...
    def get_result(cls):
        ...
        cls.df['time_diff'] = cls.df[['time_diff', 'transact']].apply(lambda x: \
                                    0 if 'Начало лога' in x.transact else x.time_diff, axis=1)
        return cls.df
